# Forgekit — Claude Code Project Forge MCP Server
...
| Customization | Edit files | Edit files | User overrides via .forgekit.json |
...
│  │  User overrides: .forgekit.json in project      │ │
│  └─────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────┘
```
  ...
  "custom_hooks": [
    {
      "name": "phi-scanner",
      "trigger": "pre-commit",
      "script": ".forgekit/hooks/phi-scanner.sh"
    }
  ],
  "custom_mcp_servers": {
    "codeseeker": {
...
3. **Override without forking.** `.forgekit.json` lets users customize thresholds,
...
# Or globally:
npm install -g @forgekit/mcp-server
claude mcp add forgekit -- forgekit-mcp
```

### GitHub Repository
```
github.com/[org]/forgekit
...
│       ├── defi-strategy/
│       └── browser-game/
└── examples/               # .forgekit.json examples for different project types
```
...
- Third-party tag packages: `@forgekit/tag-flutter`, `@forgekit/tag-rust-backend`
...
3. **Hacker News** — submit as "Show HN: Forgekit — MCP server that makes CC produce production code"
...
- "Forgekit: the missing infrastructure layer for Claude Code projects"
...
|------|---------------|---------|------|
| forgekit | Check | forgekit.dev | Building/crafting, toolkit |
...
**Recommendation: `forgekit`** — short, memorable, implies building/forging, not taken
(verify before committing). "Forge" = creating something strong from raw materials.
"Kit" = toolkit, extensible.
